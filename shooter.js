App = function()
{
    ...
    this.init  = function()
    {
        ...
        for (var i=0; i<15; i++)
        {
            ...
            var starSprite = new Sprite('images/star.png', 50);
    ...
    this.startGame = function()
    {
            ...
            for (var i=activeBullets.length-1; i>=0; i--)
            {
                ...
                for (var j=0; j<colliders.length; j++)
                {
                    if (colliders[j].isEnemy)
                    {
                        ...
                        score += 10;
						speed += 10;
                        scoreCounter.getSprite().setText("Score: " + score);
						speedDisplay.getSprite().setText("Speed: " + speed);
		...
		var speedSprite = new TextSprite("Speed: " + speed.toString(), '32px Verdana', '#f88', 'left');
        scoreCounter = new SceneObject(scoreSprite, 0, wade.getScreenWidth() / 2 - 10, -wade.getScreenHeight() / 2 + 30);
		speedDisplay = new SceneObject(speedSprite, 0, -wade.getScreenWidth() / 2, wade.getScreenHeight() / 2 -10);
        wade.addSceneObject(scoreCounter);
		wade.addSceneObject(speedDisplay);
        // spawn enemies
        enemyDelay = 2000;
